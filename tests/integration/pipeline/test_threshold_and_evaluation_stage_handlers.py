...
def test_threshold_construction_computes_the_exact_shared_mean_of_client_quantiles(tmp_path: Path) -> None:
    ...
    calibration_node_key = IdentityBuilder.calibration_score_node_key(calibration_context)
    ...
    _commit(
        repository,
        app.config,
        node_path(calibration_node_key),
...
def test_operating_point_evaluation_computes_exact_confusion_counts(tmp_path: Path) -> None:
    ...
    test_score_node_key = IdentityBuilder.test_score_node_key(test_score_context)
    ...
    _commit(
        repository,
        app.config,
        node_path(test_score_node_key),
        IdentityBuilder.test_scores_key(test_score_context),
        test_frame,
    )

    outcome = OperatingPointEvaluationStageHandler(app.config, repository).execute(eval_job)

    assert outcome.status is JobExecutionStatus.SUCCESS
    read = repository.read(node_path(eval_job.node_key))
    assert read.found and read.payload_bytes is not None
...
def test_operating_point_evaluation_rejects_a_threshold_artifact_with_foreign_provenance(tmp_path: Path) -> None:
    ...
    _commit(
        repository,
        app.config,
        node_path(threshold_job.node_key),
    ...
    _commit(
        repository,
        app.config,
        node_path(IdentityBuilder.test_score_node_key(test_score_context)),
        IdentityBuilder.test_scores_key(test_score_context),
    ...
    assert outcome.error_message is not None and "scientific fingerprint" in outcome.error_message.lower()
