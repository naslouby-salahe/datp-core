...
def _metadata(**overrides: object) -> ArtifactCommitMetadata:
    ...
    defaults: dict[str, object] = {
        ...
        "relative_path": "experiments/test/preflight",
        "parents": (),
...
def test_bytes_commit_succeeds_and_produces_correct_layout(tmp_path: Path) -> None:
    ...
    target = tmp_path / "experiments/test/preflight"
...
def test_file_commit_succeeds_and_copies_payload(tmp_path: Path) -> None:
    ...
    assert result.success
    target = tmp_path / "artifacts/experiments/test/preflight"
    assert target.is_dir()
...
def test_self_parent_is_rejected(kind: str, tmp_path: Path) -> None:
    key = ArtifactKey(node_key=StageNodeKey(experiment=ExperimentId("test"), stage=StageKind.PREFLIGHT, seed=1), kind=ArtifactKind.REPORT)
    result = AtomicArtifactRepository(tmp_path, lock_timeout=1.0).commit(
        _request(
            kind,
            tmp_path,
...
def test_duplicate_parent_is_rejected(kind: str, tmp_path: Path) -> None:
    dup_key = ArtifactKey(node_key=StageNodeKey(experiment=ExperimentId("test"), stage=StageKind.PREFLIGHT, seed=2), kind=ArtifactKind.MATERIALIZED_DATASET)
...
def test_byte_and_file_checksums_match_for_identical_content(tmp_path: Path) -> None:
    content = b"identical"
    repo = AtomicArtifactRepository(tmp_path, lock_timeout=1.0)
    bytes_result = repo.commit(_bytes_request(content, relative_path="experiments/test/bytes-path"))
    source = tmp_path / "identical.dat"
    source.write_bytes(content)
    file_result = repo.commit(_file_request(str(source), relative_path="experiments/test/file-path"))
...
def test_manifest_bytes_are_identical_for_equivalent_metadata(tmp_path: Path) -> None:
    ...
    repo = AtomicArtifactRepository(tmp_path, lock_timeout=1.0)
    repo.commit(_bytes_request(content, relative_path="experiments/test/manifest-a"))
    repo.commit(_bytes_request(content, relative_path="experiments/test/manifest-b"))

    manifest1_bytes = (tmp_path / "experiments/test/manifest-a/manifest.json").read_bytes()
    manifest2_bytes = (tmp_path / "experiments/test/manifest-b/manifest.json").read_bytes()
...
def test_manifest_round_trips_all_fields(tmp_path: Path) -> None:
    ...
    key = ArtifactKey(node_key=StageNodeKey(experiment=ExperimentId("test"), stage=StageKind.PREFLIGHT, seed=3), kind=ArtifactKind.STATISTICAL_SUMMARY)
    parent_key = ArtifactKey(node_key=StageNodeKey(experiment=ExperimentId("test"), stage=StageKind.PREFLIGHT, seed=4), kind=ArtifactKind.MATERIALIZED_DATASET)
    request = ArtifactCommitRequest(
        metadata=ArtifactCommitMetadata(
            ...
            relative_path="experiments/test/preflight/full",
            parents=(
                ArtifactParent(
                    parent_key=parent_key,
                    parent_relative_path="experiments/test/preflight/parent",
                    scientific_fingerprint=scientific,
                    execution_fingerprint=execution,
                ),
            ),
    ...
    decoded = decode_manifest((tmp_path / "experiments/test/preflight/full/manifest.json").read_bytes())
    assert decoded.artifact_key == key
    assert decoded.artifact_format == ArtifactFormat.JSON
    assert decoded.state.value == "frozen"
    assert decoded.relative_path == "experiments/test/preflight/full"
    ...
    assert decoded.parents[0].parent_relative_path == "experiments/test/preflight/parent"
...
def test_payload_filename_uses_format_value(tmp_path: Path) -> None:
    ...
    assert result.success
    assert (tmp_path / "experiments/test/preflight/payload.parquet").exists()
...
def test_payload_filename_for_safetensors_format(tmp_path: Path) -> None:
    ...
    assert result.success
    assert (tmp_path / "experiments/test/preflight/payload.safetensors").exists()
...
def test_repository_read_returns_payload_bytes_for_bytes_commit(tmp_path: Path) -> None:
    ...
    result = repository.read("experiments/test/preflight")
    assert result.found
...
def test_repository_read_returns_not_found_for_missing(tmp_path: Path) -> None:
    repository = AtomicArtifactRepository(tmp_path, lock_timeout=1.0)
    result = repository.read("experiments/test/nonexistent")
...
def test_repository_inspect_does_not_load_payload_bytes(tmp_path: Path) -> None:
    ...
    result = repository.inspect("experiments/test/preflight")
    assert result.found
    assert result.manifest is not None
...
def test_corruption_manifest_missing(tmp_path: Path) -> None:
    ...
    (tmp_path / "experiments/test/preflight/manifest.json").unlink()
    result = repository.inspect("experiments/test/preflight")
    assert not result.found
...
def test_corruption_payload_missing(tmp_path: Path) -> None:
    repository = AtomicArtifactRepository(tmp_path, lock_timeout=1.0)
    assert repository.commit(_bytes_request()).success
    (tmp_path / "experiments/test/preflight/payload.text").unlink()
    result = repository.inspect("experiments/test/preflight")
    assert not result.found
...
def test_corruption_checksum_mismatch(kind: str, tmp_path: Path) -> None:
    ...
    (tmp_path / "experiments/test/preflight/payload.text").write_bytes(b"tampered")
    result = repository.inspect("experiments/test/preflight")
    assert not result.found
...
def test_corruption_schema_incompatible(tmp_path: Path) -> None:
    repository = AtomicArtifactRepository(tmp_path, lock_timeout=1.0)
    assert repository.commit(_bytes_request()).success
    manifest_path = tmp_path / "experiments/test/preflight/manifest.json"
...
def test_failure_before_replace_leaves_no_visible_partial_artifact(kind: str, tmp_path: Path) -> None:
    ...
    target_dir = tmp_path / "experiments/test/preflight"
...
def test_successful_commit_produces_exactly_manifest_and_payload(tmp_path: Path) -> None:
    ...
    assert result.success
    contents = set(p.name for p in (tmp_path / "experiments/test/preflight").iterdir())
...
def test_fingerprints_are_identical_for_bytes_and_file_with_same_metadata(tmp_path: Path) -> None:
    ...
    source.write_bytes(content)
    repo = AtomicArtifactRepository(tmp_path, lock_timeout=1.0)
    bytes_result = repo.commit(_bytes_request(content, relative_path="experiments/test/bytes"))
    file_result = repo.commit(_file_request(str(source), relative_path="experiments/test/file"))
    assert bytes_result.manifest is not None
    assert file_result.manifest is not None
...
def test_strict_manifest_decode_is_identical_for_bytes_and_file(tmp_path: Path) -> None:
    ...
    source.write_bytes(content)
    repo = AtomicArtifactRepository(tmp_path, lock_timeout=1.0)
    repo.commit(_bytes_request(content, relative_path="experiments/test/strict/bytes"))
    repo.commit(_file_request(str(source), relative_path="experiments/test/strict/file"))

    bytes_decoded = decode_manifest((tmp_path / "experiments/test/strict/bytes/manifest.json").read_bytes())
    file_decoded = decode_manifest((tmp_path / "experiments/test/strict/file/manifest.json").read_bytes())
