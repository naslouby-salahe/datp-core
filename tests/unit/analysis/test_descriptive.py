...
def test_descriptive_summary_preserves_available_unavailable_and_excluded_counts() -> None:
    ...
    counts = ObservationCounts(unavailable=2, excluded=1)
    ...
    assert summary.statistics.mean == 0.5
    assert summary.statistics.median == 0.5
    assert summary.statistics.spread == 1.0
    assert (summary.available_count, summary.counts.unavailable, summary.counts.excluded) == (3, 2, 1)
