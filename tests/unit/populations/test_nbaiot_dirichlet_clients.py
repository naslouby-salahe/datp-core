...
def test_dirichlet_constructs_twenty_clients_and_conserves_rows(nbaiot_canonical_root: Path) -> None:
    ...
    assert diagnostics.document.partition_kind is ControlledPartitionKind.DIRICHLET
...
def test_iid_is_separate_typed_condition_and_is_deterministic(nbaiot_canonical_root: Path) -> None:
    ...
    assert first_diag.document.partition_kind is ControlledPartitionKind.IID
    assert first_diag.document.concentration is None
    ...
    assert first_diag.document.client_row_counts == second_diag.document.client_row_counts
