...
from datp_core.data.populations.contracts import DIRICHLET_CONCENTRATIONS, POPULATIONS


def test_population_capabilities_and_dirichlet_grid() -> None:
    assert len(POPULATIONS) == 5
    assert {population.id: population.client_count.value for population in POPULATIONS} == {
        PopulationId.NBAIOT_NATURAL_DEVICES: 9,
        PopulationId.NBAIOT_DIRICHLET_CLIENTS: 20,
        PopulationId.CICIOT_FILE_CLIENTS: 63,
        PopulationId.EDGE_SENSOR_CLIENTS: 10,
        PopulationId.EDGE_TEMPORAL_CLIENTS: 9,
    }
    by_id = {population.id: population for population in POPULATIONS}
    assert not by_id[PopulationId.NBAIOT_DIRICHLET_CLIENTS].requires_family_taxonomy
    ...
    assert by_id[PopulationId.EDGE_SENSOR_CLIENTS].identity_kind is PopulationIdentityKind.SOURCE_DEFINED_SENSOR_GROUPS
    assert by_id[PopulationId.EDGE_TEMPORAL_CLIENTS].identity_kind is PopulationIdentityKind.VERIFIED_TEMPORAL_GROUPS
    assert (
