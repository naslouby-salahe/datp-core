...
def test_forward_output_shape_equals_input_shape() -> None:
    model = FederatedAutoencoder((4, 3, 2, 3, 4))
...
def test_initialization_is_deterministic_from_the_seed() -> None:
    ...
    first = build_federated_autoencoder(protocol, initialization_seed=Seed(7))
    second = build_federated_autoencoder(protocol, initialization_seed=Seed(7))
...
def test_different_seeds_produce_different_initializations() -> None:
    protocol = AutoencoderProtocol(widths=(4, 3, 2, 3, 4))
    first = build_federated_autoencoder(protocol, initialization_seed=Seed(1))
    second = build_federated_autoencoder(protocol, initialization_seed=Seed(2))
    mismatched = any(
...
def test_clone_and_load_state_round_trips_exactly() -> None:
    protocol = AutoencoderProtocol(widths=(4, 3, 2, 3, 4))
    model = build_federated_autoencoder(protocol, initialization_seed=Seed(3))
    cloned = clone_autoencoder_state(model)
    other = build_federated_autoencoder(protocol, initialization_seed=Seed(4))
