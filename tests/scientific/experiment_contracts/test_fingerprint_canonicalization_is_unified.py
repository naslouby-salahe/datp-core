...
path (``unstructure_projection`` of pure domain records) -- never a second, Pydantic-specific
``model_dump`` call -- so there is exactly one authority for canonical serialization."""
...
def test_threshold_policy_projection_uses_domain_field_names_not_pydantic_aliases() -> None:
    # Domain projection must use domain field names, never Pydantic JSON-mode aliases.
    ...
    # Assert unstructured projection is a plain dict keyed by exact domain field names,
    # with no '$defs'/schema artifacts.
    projected = unstructure_projection(policy)
    assert isinstance(projected, dict)
    assert "$defs" not in projected
    assert isinstance(policy, BaseModel)
    assert set(projected) == set(type(policy).model_fields)
