...
def _analyze_cluster_ablation(
    ...
    *,
    ...
    ref_label = (
        EvaluationLabel(analysis.reference_evaluation)
    ...
    for seed in context.seeds:
        ...
        if cl_col not in ref_frame.columns or ref_frame[cl_col].null_count() > 0:
            raise ArtifactMissingError(
                f"Cluster labels are unavailable for reference seed {seed.value}"
            )
        ...
        for subset in subsets:
            abl_ctx = context.evaluation_context(
                source_label, seed, fingerprint_features=subset
            )
            abl_frame = context.artifacts.thresholds(abl_ctx)

            if cl_col not in abl_frame.columns or abl_frame[cl_col].null_count() > 0:
                raise ArtifactMissingError(
                    f"Cluster labels are unavailable for ablated seed {seed.value}"
                )
    ...
    *,
    ...
    for seed in context.seeds:
        ...
        if cl_col not in threshold_frame.columns:
            raise ArtifactMissingError(
                f"Cluster labels are unavailable for seed {seed.value}"
            )
        if threshold_frame[cl_col].null_count() > 0:
            raise ArtifactSchemaViolationError(
                f"Cluster labels contain nulls for seed {seed.value}"
            )

        # Validate unique client_ids
        if threshold_frame[cid_col].is_duplicated().any():
            raise ArtifactSchemaViolationError(
                f"Duplicate client_id in threshold frame for seed {seed.value}"
            )
        ...
        for lbl_val in unique_labels:
            ...
            if ClusterLabel(label_str) not in expected_set:
                raise ScientificContractViolationError(
                    f"Cluster label '{label_str}' is outside configured range "
        ...
        memberships = tuple(
            ClientClusterMembership(
                ...
                cluster_label=ClusterLabel(str(int(cluster_label))),
            )
            for client_id, cluster_label in membership_frame.rows()
        )

        # Cluster sizes via group_by
        sizes_df = (
            threshold_frame.group_by(cl_col)
            .agg(pl.len().alias("client_count"))
        )
        counts_map: dict[ClusterLabel, int] = {lbl: 0 for lbl in expected_labels}
        for row in sizes_df.rows():
            counts_map[ClusterLabel(str(int(row[0])))] = row[1]

        cluster_sizes_tuple = tuple(
            ClusterSize(
                cluster_label=lbl, client_count=counts_map[lbl]
            )
            for lbl in expected_labels
        )

        # Singleton / empty flags from the group-by result
        singleton_cluster_flag = (
            sizes_df.filter(pl.col("client_count") == 1).height > 0
        )
        ...
        if metric_frame[cid_col].is_duplicated().any():
            raise ArtifactSchemaViolationError(
                f"Duplicate client_id in metric frame for seed {seed.value}"
            )

        unknown_metric_clients = metric_frame.select(cid_col).filter(
            ~pl.col(cid_col).is_in(threshold_frame[cid_col])
        )
        if unknown_metric_clients.height > 0:
            raise PopulationAlignmentError(
                f"Metric frame references clients outside the threshold population "
                f"for seed {seed.value}: "
                f"{sorted(unknown_metric_clients[cid_col].to_list())}"
            )
        ...
        if joined.height != threshold_frame.height:
            raise PopulationAlignmentError(
                f"Threshold/metric join changed client population for seed {seed.value}"
            )

        # Count clients with any FPR status
        metric_covered = (
            joined.filter(pl.col(fpr_status_col).is_not_null()).height
        )
        ...
        has_non_finite = fpr_value_frame.filter(
            pl.col("value").is_not_null() & (~pl.col("value").is_finite())
        ).height > 0
        if has_non_finite:
            raise ArtifactSchemaViolationError(
                "Non-finite false-positive-rate in cluster stability "
                f"for seed {seed.value}"
            )
    ...
    for seed in sorted_seeds[1:]:
        ...
        if not cur_frame[cid_col].series_equal(ref_client_ids):
            raise PopulationAlignmentError(
                "Incompatible client populations across seeds for ARI computation"
            )
    ...
    if len(aris) != expected_pair_count:
        raise ScientificContractViolationError(
            f"ARI pair count mismatch: expected {expected_pair_count}, "
            f"got {len(aris)}"
        )
