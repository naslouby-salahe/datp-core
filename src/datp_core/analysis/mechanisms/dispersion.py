...
class GroupDispersionObservation(StrictModel):
    ...
    def validate_observation(self) -> "GroupDispersionObservation":
        if not self.thresholds or not self.false_positive_rates:
            raise ValueError(
                "grouped dispersion requires threshold and FPR observations"
            )
        if len(self.thresholds) != len(self.false_positive_rates):
            raise ValueError(
                "grouped threshold and FPR observations must cover the same clients"
            )
        return self
...
class GroupedDispersionResult(StrictModel):
    ...
    def validate_result(self) -> "GroupedDispersionResult":
        ...
        if (
            len(self.within_group_threshold_spreads) != group_count
            or len(self.within_group_fpr_spreads) != group_count
        ):
            raise ValueError(
                "grouped dispersion requires one threshold and FPR spread per group"
            )
        ...
        if (
            self.singleton_groups != expected_singletons
            or self.empty_groups != expected_empty
        ):
        ...
        if self.availability is AvailabilityStatus.AVAILABLE:
            if (
                ...
                raise ValueError(
                    "available grouped dispersion requires complete values and no reason"
                )
        elif self.reason is None:
            raise ValueError(
                "unavailable grouped dispersion requires an explicit reason"
            )
        return self
...
def grouped_dispersion(
    ...
    ordered = tuple(
        sorted(observations, key=lambda item: item.group_index.value)
    )
    if tuple(item.group_index.value for item in ordered) != tuple(
        range(len(ordered))
    ):
        raise ValueError("group indexes must be consecutive from zero")
    threshold_means = tuple(
        float(np.mean([value.value for value in item.thresholds]))
        for item in ordered
    )
    fpr_means = tuple(
        float(np.mean([value.value for value in item.false_positive_rates]))
        for item in ordered
    )
    group_sizes = tuple(
        PairedObservationCount(len(item.thresholds)) for item in ordered
    )
    return GroupedDispersionResult(
        evidence_role=EvidenceRole.MECHANISM,
        ...
        within_group_threshold_spreads=tuple(
            MetricValue(
                max(value.value for value in item.thresholds)
                - min(value.value for value in item.thresholds)
            )
            for item in ordered
        ),
        ...
        across_group_threshold_spread=MetricValue(
            max(threshold_means) - min(threshold_means)
        ),
        across_group_mean_fpr_spread=MetricValue(
            max(fpr_means) - min(fpr_means)
        ),
