    ...
    def __post_init__(self) -> None:
        if not self.scores:
            raise ScientificContractError(
                ErrorMessage("threshold diagnostics require non-empty finite held-out benign scores")
            )
        stable_row_ids = tuple(item.stable_row_id for item in self.scores)
        if len(stable_row_ids) != len(set(stable_row_ids)):
            raise ScientificContractError(
                ErrorMessage("threshold diagnostics require unique held-out stable row identities")
            )

        for item in self.scores:
            if item.client != self.client:
                raise ScientificContractError(
                    ErrorMessage("threshold diagnostics require scores from the evaluated client")
                )
