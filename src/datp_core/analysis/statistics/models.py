...
``StatisticalMethod``/``BootstrapMethod``/``StatisticalProfileRecord``/``NestedReplicatePolicyRecord``
moved to ``datp_core.config.statistical_profiles`` -- they are configuration schema (how a
statistical procedure is authored), not analysis-execution results.
"""
