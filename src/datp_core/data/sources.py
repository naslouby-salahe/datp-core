"""Shared dataset source behavior reused by all three adapters: source-tree discovery and
inventory, streaming CSV row validation, split-manifest read/encode, and benign-training-row
normalization fitting. Genuinely shared, unlike a generic dumping-ground -- every function here
is consumed by at least two adapters.
"""
...
def _inventory_source_tree(
    ...
    Uses the exact same glob/rglob logic as the dataset readiness audit's _source_files method:
