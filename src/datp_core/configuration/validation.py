...
Imports only the ResolvedProjectConfiguration type from configuration/resolution.py (for type
hints) -- never resolve_project_configuration_candidate() itself, and resolution.py never imports
this module. Neither module imports the other; only configuration/project.py imports both,
breaking the resolver/validator cycle that existed in the pre-refactor config/resolver.py <->
config/validation.py pair.
"""
