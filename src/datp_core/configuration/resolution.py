"""Staged resolution pipeline converting authored YAML documents into a ResolvedProjectConfiguration candidate.

This module defines ``ResolvedProjectConfiguration`` and builds it, but never validates it -- that
is ``configuration/validation.py``'s responsibility, invoked only by ``configuration/project.py``
(the sole caller allowed to import both, breaking the resolver/validator import cycle that existed
in the pre-refactor ``config/resolver.py`` <-> ``config/validation.py`` pair).
"""
...
from datp_core.configuration.protocol_resolution import (
...
def resolve_project_configuration_candidate(
    ...
    Callers needing a fully validated configuration must use
    ``configuration.project.resolve_project_configuration`` instead, which validates this candidate
    before returning it. This function exists so resolution never imports validation (see module
    docstring) -- it must not be called directly outside ``configuration/project.py`` and tests that
    intentionally exercise resolution in isolation from validation.
    """
    # execution_profile is required from the environment (DATP_EXECUTION_PROFILE), not a default;
    # see the matching comment in configuration/runtime_resolution.py.
