"""Pandera DataFrame schema contracts validating physical tabular artifacts flowing between stages.

Shared across learning/ (calibration/test score frames), thresholding/ (threshold frame), and
evaluation/ (client metric frame) -- genuinely a pipeline-level contract on what a stage's Parquet
payload must look like, not owned by any single feature. Confirmed heavily used (18 call sites
across the pre-refactor application layer) but previously had zero direct unit tests despite being
safety-critical (CURRENT_ARCHITECTURE.md finding) -- direct tests are added during test migration.
"""
