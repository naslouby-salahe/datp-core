...
def run_confirmatory_seed(training_seed: Seed) -> ConfirmatorySeedResult:
    ...
    methods = tuple(
        dict.fromkeys(entry.coordinate.threshold_method for entry in campaign.entries)
    )
...
def _required_metric(document, metric: MetricId) -> MetricValue:
