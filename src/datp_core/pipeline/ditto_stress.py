...
from datp_core.pipeline.construct_population import ConstructDeclaredPopulationRequest, construct_declared_population
...
def _personalized_scores(
    *,
    ...
    for owned in sorted(training.personalized_candidates.items, key=lambda item: item.client):
        ...
        manifest = generate_federated_scores(
