...
def _resolve_analysis(a: AnalysisSpecConfig) -> AnalysisRecord:
    statistical_profile = StatisticalProfileId(a.statistical_profile)
    match a:
        case PairedThresholdAnalysisConfig():
            return PairedThresholdAnalysisRecord(
                ...
                secondary_statistical_profile=(
                    StatisticalProfileId(a.secondary_statistical_profile)
                    if a.secondary_statistical_profile is not None
                    else None
                ),
                first_evaluation=a.first_evaluation,
                second_evaluation=a.second_evaluation,
                primary_metric=a.primary_metric,
                delta_orientation=a.delta_orientation,
                delta_interpretation=a.delta_interpretation,
        ...
        case AbsorptionAnalysisConfig():
            return AbsorptionAnalysisRecord(
                label=a.label,
                kind=a.kind,
                result_type=a.result_type,
                statistical_profile=statistical_profile,
                absorption_metric=a.absorption_metric,
                formula=a.formula,
        ...
        case AlertBurdenAnalysisConfig():
            return AlertBurdenAnalysisRecord(
                ...
                source_evaluations=tuple(a.source_evaluations),
                required_operational_input=a.required_operational_input,
                per_client_reporting_required=a.per_client_reporting_required,
                unavailable_behavior=a.unavailable_behavior,
            )
        case AnchorEquivalenceAnalysisConfig():
            return AnchorEquivalenceAnalysisRecord(
                label=a.label,
                kind=a.kind,
                result_type=a.result_type,
                statistical_profile=statistical_profile,
                source_analysis=a.source_analysis,
                comparison_mode=a.comparison_mode,
                comparison_mode_rule=a.comparison_mode_rule,
                interval_width_tolerance_multiplier=a.interval_width_tolerance_multiplier,
                floating_point_tolerance=a.floating_point_tolerance,
                historical_reference=a.historical_reference,
                expected_metric=a.expected_metric,
                expected_first_threshold_policy=a.expected_first_threshold_policy,
                expected_second_threshold_policy=a.expected_second_threshold_policy,
                statistical_fallback_requirements=tuple(a.statistical_fallback_requirements),
                failure_reasons=tuple(a.failure_reasons),
                downstream_blocking_behavior=a.downstream_blocking_behavior,
            )
        case ClusterStabilityAnalysisConfig():
            return ClusterStabilityAnalysisRecord(
                label=a.label,
                kind=a.kind,
                result_type=a.result_type,
                statistical_profile=statistical_profile,
                source_evaluation=a.source_evaluation,
        ...
        case DistributionMechanismAnalysisConfig():
        ...
        case LockedClientDistributionAnalysisConfig():
            return LockedClientDistributionAnalysisRecord(
                ...
                source_evaluations=tuple(a.source_evaluations),
                produced_fields=tuple(a.produced_fields),
                locked_client_identifier=a.locked_client_identifier,
            )
        case MetricAssociationAnalysisConfig():
            return MetricAssociationAnalysisRecord(
                ...
                predictor_metric=a.predictor_metric,
                outcome_metric=a.outcome_metric,
                outcome_source_analysis=a.outcome_source_analysis,
                interpretation_constraint=a.interpretation_constraint,
                grouping_dimension=a.grouping_dimension,
            )
        case QuantileEstimationAnalysisConfig():
            return QuantileEstimationAnalysisRecord(
                ...
                source_evaluations=tuple(a.source_evaluations),
                produced_fields=tuple(a.produced_fields),
                oracle_reference=a.oracle_reference,
            )
        case RecoveryFractionAnalysisConfig():
            return RecoveryFractionAnalysisRecord(
                ...
                statistical_profile=statistical_profile,
                formula=a.formula,
                numerator_analysis=a.numerator_analysis,
                denominator_analysis=a.denominator_analysis,
                denominator_composition=a.denominator_composition,
                denominator_materiality_rule=a.denominator_materiality_rule,
                undefined_denominator_behavior=a.undefined_denominator_behavior,
            )
        case ResourceCostAnalysisConfig():
            return ResourceCostAnalysisRecord(
                ...
                source_evaluations=tuple(a.source_evaluations),
                produced_fields=tuple(a.produced_fields),
                estimate_basis=a.estimate_basis,
            )
        case TemporalRecoveryAnalysisConfig():
            return TemporalRecoveryAnalysisRecord(
                ...
                statistical_profile=statistical_profile,
                primary_metric=a.primary_metric,
                static_reference_evaluation=a.static_reference_evaluation,
                frozen_evaluation=a.frozen_evaluation,
                recalibrated_evaluation=a.recalibrated_evaluation,
                recovery_fields=tuple(a.recovery_fields),
                drift_excess_formula=a.drift_excess_formula,
                recovered_amount_formula=a.recovered_amount_formula,
                recovery_ratio_formula=a.recovery_ratio_formula,
                meaningful_degradation_rule=a.meaningful_degradation_rule,
                recovery_ratio_precondition=a.recovery_ratio_precondition,
                negative_recovery_policy=a.negative_recovery_policy,
        ...
        case ThresholdStabilityAnalysisConfig():
