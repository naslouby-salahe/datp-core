...
def resolve_report_profile(identifier: str, cfg: ReportProfileConfig) -> ReportProfileRecord:
    return ReportProfileRecord(
        ...
        columns=(
            tuple(ReportColumnRecord(name=c.name, unit=c.unit, direction=c.direction) for c in cfg.columns)
            if cfg.columns is not None
            else None
        ),
        series=(
            tuple(ReportColumnRecord(name=c.name, unit=c.unit, direction=c.direction) for c in cfg.series)
