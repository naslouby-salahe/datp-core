...
Kept as its own module (parallel to config/resolution/{datasets,experiments,protocols}.py) because
it resolves exactly one authored document (runtime.yaml) into typed records, the same axis those
siblings use.
"""
...
def resolve_runtime_configuration(
    ...
    execution_profiles = {
        key: ExecutionProfileRecord(
            ...
            resource_budget=ResourceBudgetRecord(
                max_ram_gib=int(profile.resource_budget.max_ram_gib),
                max_vram_gib=profile.resource_budget.max_vram_gib,
            ),
            concurrency=ConcurrencyRecord(
                worker_count=int(profile.concurrency.worker_count),
            ...
            log_interval_rounds=int(profile.log_interval_rounds),
