"""FedAvg/FedProx federated training and Ditto model-personalization training.
...
are reused by Ditto's training loop below -- FedAvg is the core ladder algorithm, so this module
owns them; Ditto's aggregated global state plus persistent, never-aggregated per-client
personalized states are trained against a proximal objective toward the round-start global.
"""
