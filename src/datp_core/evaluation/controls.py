...
def validate_fixed_score_controls(
    ...
    *,
    auroc_absolute_tolerance: float,
) -> None:
    ...
    if auroc_absolute_tolerance < 0:
        raise ValueError("AUROC absolute tolerance must be non-negative")
...
def _require_auroc_invariance(
    first: tuple[ClientAurocEvidence, ...], second: tuple[ClientAurocEvidence, ...], tolerance: float
) -> None:
