...
def calibration_variance_terms(calibration: pl.DataFrame) -> QuantileVarianceTerms:
    ...
    for _, group in calibration.group_by("client_id", maintain_order=True):
        ...
        means_and_variances.append((group_values.size, float(
            group_values.mean()), float(np.var(group_values))))
    ...
    between = sum(count * (mean - pooled_mean) ** 2 for count,
                  mean, _ in means_and_variances) / total
