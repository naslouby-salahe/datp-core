"""Markdown and LaTeX table rendering from frozen result profiles."""
...
def render_table(profile: Mapping[str, object], results: list[Mapping[str, object]]) -> dict[str, object]:
    """Render a table from a frozen result profile and its statistical results."""
    ...
    return {
        ...
        "outputs": {"markdown": markdown, "latex": latex},
    }
...
# Strengthen LaTeX escaping beyond underscores (also escape &, %, $, #, {, }, ~, ^)
...
def _escape_latex(value: str) -> str:
    return value.translate(_LATEX_SPECIAL)
