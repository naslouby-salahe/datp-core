...
def _publish_nbaiot(raw_root: Path, canonical_root: Path) -> DatasetPublication:
    ...
    return _NBAIOT_PUBLISHER.materialize(sources, canonical_root)
...
def _publish_ciciot2023(raw_root: Path, canonical_root: Path) -> DatasetPublication:
    ...
    return _CICIOT2023_PUBLISHER.materialize(sources, canonical_root)
...
def _publish_edge_iiotset(raw_root: Path, canonical_root: Path) -> DatasetPublication:
    ...
    return _EDGE_IIOTSET_PUBLISHER.materialize(benign, attacks, canonical_root)
...
_DATASET_BINDINGS: dict[DatasetId, DatasetBinding] = {
    DatasetId.NBAIOT: DatasetBinding(NBAIOT_CAPABILITIES, NBAIOT_SCHEMA, _publish_nbaiot),
    DatasetId.CICIOT2023: DatasetBinding(CICIOT2023_CAPABILITIES, CICIOT2023_SCHEMA, _publish_ciciot2023),
    DatasetId.EDGE_IIOTSET: DatasetBinding(EDGE_IIOTSET_CAPABILITIES, EDGE_SCHEMA, _publish_edge_iiotset),
}
