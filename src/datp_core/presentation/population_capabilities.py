...
_ROWS = (
    ...
    PopulationCapabilityTableRow(
        PopulationId.CICIOT_FILE_CLIENTS, "processed CSV file pseudo-client", 63, "No", "Yes",
        "Not authorized for DATP claims", "No", "applicability boundary",
    ),
    PopulationCapabilityTableRow(
        PopulationId.NBAIOT_DIRICHLET_CLIENTS, "synthetic Dirichlet client", 20, "No", "Yes",
        "Yes, where source attack support remains valid", "No", "controlled heterogeneity sensitivity",
    ),
    PopulationCapabilityTableRow(
        PopulationId.EDGE_SENSOR_GROUPS, "benign sensor-group folder", 10, "No physical-device claim", "Yes",
        "No — valid per-client attack assignment unavailable", "No", "independent external benign-equity validation",
    ),
    PopulationCapabilityTableRow(
        PopulationId.EDGE_TEMPORAL_GROUPS, "timestamp-valid sensor-group folder", 9, "No physical-device claim", "Yes",
        "No — temporal experiment is benign-only", "Yes", "one-shot temporal boundary",
    ),
)
...
_ROADMAP_POPULATION_LABELS = {
    ...
    PopulationId.EDGE_SENSOR_GROUPS: "EDGE_SENSOR_CLIENTS",
    PopulationId.EDGE_TEMPORAL_GROUPS: "EDGE_TEMPORAL_CLIENTS",
}
