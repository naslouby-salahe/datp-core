...
| Claim | Evidence | Regime | Metric | Min pass | Placement |
|---|---|---|---|---|---|
| The threshold-scope effect generalizes to an independent device-partitioned dataset | B1–B4 + q-sensitivity + `B-FedStatsBenign` on Edge-IIoTset | Regime D | CV(FPR) + BCa CI | Effect direction consistent with Regime A, or divergence reported as boundary | Main |
...
| Device-aware thresholds under a chronological split behave per one of three pre-specified temporal outcomes | Edge-IIoTset chronological 70/30; frozen vs one-shot recalibration | Regime D-temporal | Per-window CV(FPR); recovery ratio | One pre-specified outcome applied (§11.1) | Main |
...
- **RQ6 (external / boundary).** Does the threshold-scope effect generalize to an independent device-partitioned dataset (Edge-IIoTset), how does it behave across Dirichlet severity, and where does it fail (near-homogeneous file-level pseudo-clients; chronological drift)?
...
**2×2 reconciliation.** The full independent 2×2 with cost accounting is the *spin-off*. In this paper the 2×2 is realized economically as a single model-personalization comparator (Ditto or a `FedRep-AE`/`FedPer-AE` fallback) crossed with the threshold ladder, so that {FedAvg, personalized} × {B1, B2} populates the 2×2 corners and the absorption ratio reads directly off it. This satisfies the 2×2 conceptual requirement without exceeding the hard limit of one model-personalization stress test.
...
| E-B1 | Temporal recalibration MVE | Regime D chronological | frozen vs one-shot | Per-window CV(FPR), recovery ratio | 70/30 split | + per-client slope | Regime D timestamps | Tier 6 | One of three outcomes (§11.1) | Main |
...
| E-Q4 | Bootstrap CIs for all secondary metrics; full Wilcoxon + Cliff's δ | Effect sizes | Supplement |
...
- Wilcoxon signed-rank and Cliff's δ are descriptive secondary evidence only.
...
Edge-IIoTset is the sole temporal substrate (multi-day capture window). N-BaIoT drift magnitude is limited; CICIoT2023 temporal probing is rejected (`TEMPORAL_REJECTED_NO_TIMESTAMPS`). The MVE is a chronological 70/30 split: AE training and B1/B2/B4 calibration use the first 70% of each client's benign data by capture time; evaluation on the last 30% reports frozen thresholds and one-shot recalibration at the temporal boundary. Rejected as scope drift: streaming sliding-window recalibration, cross-dataset transfer, Page-Hinkley, FLARE/FLAME, Byzantine-robust federated conformal.
...
- **Confirms.** "On Edge-IIoTset ([partition], K = [x]), B2 reduces CV(FPR) from [Y] to [Z] (95% BCa CI [a, b]), consistent with Regime A."
