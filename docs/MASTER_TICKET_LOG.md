...
## N. Phase 2 anchor recovered-semantics register (P2-T001 / P2-T002)

Produced by `P2-T001` (read-only inspection of the behavioral-reference repository `/home/naslouby/Projects/datp`) and recorded here by `P2-T002`, per both tickets' "no separate register file" rule. This section supersedes the M.4 assumption that the historical anchor checkpoint protocol "remains open pending recovery" for the schedule/selection question specifically — see N.2, accepted provisionally — while leaving every other listed M.4 blocker untouched. No file in `/home/naslouby/Projects/datp` was executed, imported, copied, or used as a layout/migration template; every row below cites an exact file path and line range as evidence. Where architecture/roadmap text conflicts with recovered evidence, the conflict is recorded, not silently resolved by editing Architecture or Roadmap (P2-T001/P2-T002 are not authorized to edit those documents).

**Revision note (2026-07-15, second pass).** An initial version of this register reported all 13 semantics as "confirmed matching" and zero print-grade blockers. That was an overclaim: item 7 (FedAvg local-epoch count) was recorded as "RESOLVED as a recovered fact" on the strength of the repository default and code alone, without tracing whether that default was the value actually used to produce the published historical anchor results, and without checking it against the published paper text. A deeper provenance trace (N.2b) found the repository default and every proven-executed Regime A run artifact agree with each other (`E=5`) but conflict with the published paper's stated methodology (`E=1`). This is now carried as an explicit print-grade blocker, `ANCHOR_LOCAL_EPOCH_PROTOCOL_UNRESOLVED`, rather than a resolved fact. `P2-T001`/`P2-T002` were reopened to correct this before Phase 2 proceeded further.

### N.1 Recovered semantics table

| # | Subject | Recovered value | Evidence (`/home/naslouby/Projects/datp/...`) | Roadmap/Architecture representation | Justified difference | Blocker state |
|---|---|---|---|---|---|---|
| 1 | AE architecture | Symmetric FC autoencoder; encoder `Linear→[BatchNorm1d if use_bn]→activation` per layer; decoder mirrors reversed dims, final layer no activation; `encoder_dims=[80,40,20]` (Regime A/C), `input_dim=115` (N-BaIoT); `use_bn=false` by default; loss `MSE`; per-sample score `((x-x̂)**2).mean(dim=1)` | `src/datp/models/autoencoder.py:11-74`; `src/datp/conf/config.yaml:5-12` | Arch §27.1: "mirrored ReLU autoencoder with encoder widths (80,40,20), no BatchNorm, MSE loss" | None — matches | RESOLVED |
| 2 | Optimizer | Adam, `lr=0.001`, no weight decay anywhere in repo (exhaustive grep, zero matches) | `src/datp/training/fl/client.py:58`; `src/datp/baselines/common/training.py:124-125`; `src/datp/conf/config.yaml:8` | Arch §27.1: "Adam at learning rate 0.001" | None — matches | RESOLVED |
| 3 | Scheduler | Absent. No LR scheduler anywhere in the repo (exhaustive grep, zero matches); `configure_optimizers` returns a bare optimizer | `src/datp/baselines/common/training.py:124-125`; `src/datp/training/fl/client.py:58` | Arch §27.1: "no learning-rate scheduler" | None — matches | RESOLVED |
| 4 | Preprocessing | `sklearn.StandardScaler`, fit strictly per-client on that client's own TRAIN split only, applied unchanged to that client's CAL/TEST_BENIGN/TEST_ATTACK | `src/datp/data/scaling.py:21-34`; `src/datp/data/datasets/nbaiot/prepare.py:131-140,201-204` | Arch §27 lists "Inherited preprocessing strategy/scope" as a genuine blocker needing behavioral-reference evidence | Recovered evidence closes this Arch §27 blocker: TRAIN-only, per-client fit scope | RESOLVED (closes an Arch §27 listed blocker) |
| 5 | Physical-device clients | Exactly 9 hardcoded device directories, 1:1 to N-BaIoT sub-directories; `NUM_DEVICES=len(DEVICE_DIRS)`; device→family map (doorbell/camera/other) used by B3 | `src/datp/data/datasets/nbaiot/spec.py:17-41`; `src/datp/data/datasets/nbaiot/prepare.py:91-204` | Road/Arch: Regime A = natural device split, K=9 | None — matches | RESOLVED |
| 6 | Train/cal/test split | Chronological, contiguous-gapped, in file order: train 0.60 / gap 0.01 (discarded) / cal 0.20 / gap 0.01 (discarded) / test_benign ≈0.18–0.19; attacks scored separately as test_attack; CAL strictly benign-only; `n_min=100` on CAL count; below threshold → "Calibration-Pending", not dropped, falls back to τ_global downstream | `src/datp/data/datasets/nbaiot/spec.py:14-15,48-53`; `prepare.py:39-70,121-129`; `conf/config.yaml:16`; `baselines/common/eligibility.py:19-30,75-83` | Blueprint.md:267 ("N-BaIoT split. Chronological: 60% train / 1% gap / 20% calibration / 1% gap / ~19% test."); Arch/Road benign-only calibration, `n_min=100` | None — matches | RESOLVED |
| 7 | FedAvg weighting/E/participation | Flower's standard `FedAvg` aggregation weighted by each client's returned `n_train` (unmodified base `aggregate_fit`); full participation every round (`fraction_fit=fraction_evaluate=1.0`, `min_*_clients=num_clients`); batch size 256; rounds: `rounds_initial=40`, `rounds_max=150` (amended from an original T=50/T_max=75 heuristic per a documented protocol amendment — zero convergences observed within 75 rounds; convergence observed at ~103–130 rounds) — all RESOLVED, matching. **Local epoch count E is NOT resolved — see N.2b.** Repository default and every proven-executed Regime A artifact show `local_epochs=5`; the published paper text and reproducibility table state `E=1`. These conflict and are not adjudicated here. | `src/datp/training/fl/strategy.py:157-177`; `src/datp/training/fl/client.py:50-81`; `src/datp/training/fl/convergence.py:74-112`; `conf/config.yaml:24-25,34-41`; `Blueprint.md:274` (protocol-amendment note); `outputs/results/a/{b0,b1,b2,b3,b4}/seed_{0-4}/resolved_config.yaml` (25 files, unanimous `local_epochs: 5`); `paper/sections/experiments.tex:46,121` (published `E=1`) | Arch §1.2/§27.1 state E=1 as the FedAvg baseline; §27.1 additionally claims E=1 is "inherited" from the reference | E is **not** a justified-difference case — it is an unresolved conflict between two evidence sources purporting to describe the same historical anchor; see N.2b | **UNRESOLVED — `ANCHOR_LOCAL_EPOCH_PROTOCOL_UNRESOLVED`, print-grade blocker, see N.2b** |
| 8 | Historical anchor checkpoint protocol | **Single checkpoint only**, saved unconditionally exactly once after the FL simulation loop ends (whichever round it stops at — early convergence per `ConvergenceMonitor` or the `rounds_max=150` hard cap); `final_params = strategy.latest_parameters` is used regardless of `converged_round`; no multi-round schedule, no candidate pool, no ranking metric, no tie-break exists in this repo | `src/datp/training/fl/runner.py:84-93,307-335` (independently re-read and confirmed directly, not only via subagent report); `src/datp/training/fl/convergence.py:74-112`; `Blueprint.md:274` ("...does not alter... the checkpoint-save policy...") | Arch §17.1 defines `CheckpointDescriptor(kind=SCIENTIFIC)` saved at rounds `{25,50,75,100,125,150,200}` with selection rule `regime_a_weighted_benign_validation_loss_v1` and tie-break `earliest_scheduled_round_v1` — but Road line 295 explicitly labels this the **"Journal checkpoint protocol (locked)"**, distinct from any anchor-specific protocol | See N.2 — this is `AnchorHistoricalCheckpointProtocol`, distinct from `JournalCheckpointProtocol`; NOT `{25,...,200}` | **RESOLVED, not `ANCHOR_CHECKPOINT_PROTOCOL_UNRESOLVED`** — see N.2 for full decision record |
| 9 | Calibration score derivation | Per-sample MSE reconstruction error via `Autoencoder.reconstruction_error`, run over CAL split of the single saved checkpoint; no aggregation at scoring time (raw per-sample array persisted) | `src/datp/models/autoencoder.py:65-71`; `src/datp/training/fl/scoring.py:44-48,98-137` | Consistent with Road/Arch reconstruction-error scoring | None | RESOLVED |
| 10 | Test (benign/malicious) score derivation | Identical formula/code path as item 9, applied to TEST_BENIGN and TEST_ATTACK tensors in the same loop; no branch by label | `src/datp/training/fl/scoring.py:113-137` | Consistent | None | RESOLVED |
| 11 | Threshold formulas B1–B4 | B1: unweighted arithmetic mean of eligible per-client `percentile(E_i, q)`, `q=0.95`, applied globally including to Calibration-Pending clients. B2: per-client local `percentile(E_i, q)` directly, Pending clients fall back to τ_global. B3 (Regime A only): unweighted mean of member τ_i within each of 3 fixed device families (doorbell/camera/other). B4: fingerprint `[mean(E_i), std(E_i,ddof=1), skew(E_i), percentile(E_i,q)]`, `StandardScaler`, `KMeans`, Regime A fixed `k=3` default, cluster τ = mean of member τ_i; Pending clients never cluster | `src/datp/baselines/common/eligibility.py:44-52`; `src/datp/baselines/main/b1.py,b2.py,b3.py,b4.py` (full files); `conf/config.yaml:43-50`; `Blueprint.md:260-278` | Arch/Road: B1 unweighted mean, B2 local p95, B3 authorized-taxonomy family mean, B4 exact k-means++ on `[mean,std,skew,p95]`, canonical K=3 | None — formulas match; family taxonomy = doorbell/camera/other (3 families) is the recovered authorized taxonomy for B3 | RESOLVED |
| 12 | Evaluation formulas | Per-client confusion via `score > τ`; `FPR=FP/(FP+TN)`, `TPR=TP/(TP+FN)`, `math.nan` on zero-denominator (no silent zero substitution); `CV(FPR)=std(ddof=1)/mean` over eligible clients as a first-class primary metric, plus `CV(TPR)`, IQR, worst-client FPR/BA, `p10(macro-F1)`; AUROC/PR-AUC via sklearn, used only as (a) a B0-only sanity gate (`b0_auroc<0.90`→warn) and (b) a reported/compared metric — never the threshold-policy decision criterion | `src/datp/evaluation/metrics.py:43-73,134-245`; `src/datp/statistics/cv.py:10-18`; `src/datp/evaluation/ranking.py:15-25`; `src/datp/audit/results.py:977-996` | Arch/Road: CV(FPR) primary, AUROC control-only | None — matches | RESOLVED |
| 13 | Interpretation semantics | Confirmatory pass/fail = whether the bootstrap CI on the per-seed delta `CV(FPR)[B1]-CV(FPR)[B2]` excludes zero (`excludes_zero=(ci_lower>0.0) or (ci_upper<0.0)`); a separate GO/CONTINGENCY diagnostic (`CV(FPR)_B1 > 0.10`) gates whether the full experiment matrix proceeds — it is a preconditon signal, not the confirmatory verdict itself | `src/datp/pipeline/diagnostic.py:227-254`; `src/datp/statistics/bootstrap.py:19-51`; `src/datp/config/models.py:106` | Road §5.1 Tier-1: BCa CI on Δ_s excludes zero, positive direction | None — matches (reference used a percentile bootstrap, `n_bootstrap=10000`, `ci=0.95`, `bootstrap_seed=42`; Road/Arch require BCa specifically for the new confirmatory work — a pre-existing, already-locked upgrade, not a conflict requiring resolution here) | RESOLVED |

### N.2 `AnchorHistoricalCheckpointProtocol` (distinct from `JournalCheckpointProtocol`)

- **Historical checkpoint rounds.** Exactly one, unscheduled: whichever round the FL loop terminates at (early-convergence detection, else hard cap `rounds_max=150`, itself amended up from an original `T_max=75` heuristic — Blueprint.md:274). No fixed list of rounds exists.
- **Candidate eligibility rule.** N/A — there is never more than one candidate; no candidate pool is ever constructed or persisted.
- **Ranking metric.** N/A — no ranking is performed.
- **Ranking population.** N/A.
- **Ranking direction.** N/A.
- **Tie-break.** N/A — impossible with a single candidate.
- **Prohibited selection inputs.** N/A (nothing selects among candidates because only one state is ever saved).
- **Evidence locations.** `src/datp/training/fl/runner.py:84-93` (`_save_checkpoint`, single unconditional `torch.save`), `runner.py:307-335` (`_save_checkpoint(model, ckpt_dir)` called exactly once, after the simulation loop, using `strategy.latest_parameters` regardless of `converged_round`); `src/datp/training/fl/convergence.py:74-112` (stop signal only, no checkpoint-selection logic); `Blueprint.md:274` ("This amendment does not alter... the checkpoint-save policy..." — corroborates the save policy was a deliberate, stable design choice, not an omission). Independently re-verified by direct read of `runner.py` (not solely the P2-T001 subagent report) given the significance of this finding.
- **Uncertainty.** None on the reference repo's own behavior — confirmed by exhaustive keyword grep (`checkpoint`, `anchor`, `historical`) and an explicit literal-sequence grep for `25,50,75,100,125,150,200` in any form, both returning zero matches for any multi-round anchor schedule anywhere in `/home/naslouby/Projects/datp` (excluding its `.venv`).
- **Blocker state.** **RESOLVED — not `ANCHOR_CHECKPOINT_PROTOCOL_UNRESOLVED`.** The historical anchor protocol is recovered with high confidence from two independent sources (source code and `Blueprint.md` design documentation). Per the governing constraint that the journal schedule `{25,50,75,100,125,150,200}` must never be imposed on the anchor route without evidence it was actually used historically — and the recovered evidence proves the opposite (a single, unranked, unscheduled checkpoint) — `P2-T010` (federated anchor checkpoint schedule and selection) must implement the ANCHOR route as: train to convergence-or-cap under the recovered `rounds_initial=40`/`rounds_max=150` convergence policy, and persist exactly one `CheckpointDescriptor(kind=SCIENTIFIC)` for the terminal round, with no ranking/selection step (a `CheckpointSelector` over a singleton candidate set, if the typed mechanism from Arch §17.1/P1-T022 is reused for type-uniformity, must degrade to a no-op "the one candidate is selected" rule — it must not silently reuse `regime_a_weighted_benign_validation_loss_v1` or the `{25,...,200}` schedule, both of which are `JournalCheckpointProtocol`-only per Road:295).
- **Affected identities.** Anchor `TrainingSpec`/checkpoint schedule value objects, `CheckpointSelectionSpec`, `CheckpointDescriptor` population for `ProtocolTrack.DATP_ANCHOR`.
- **Affected tickets.** `P2-T008` (training spec), `P2-T009` (FedAvg training round/convergence policy), `P2-T010` (checkpoint schedule and Regime-A selection — primary owner of this decision; must implement a single termination-state checkpoint with no ranking and no tie-break, per this decision, accepted provisionally), `P2-T020` (readiness evaluator must cite this resolution).

### N.2b `AnchorLocalEpochProtocol` — UNRESOLVED, print-grade blocker

Distinguishes three values that must not be conflated, per explicit instruction after the initial recovery pass wrongly reported them as matching:

- **Repository default** (`src/datp/conf/config.yaml:37`, `cfg.federation.local_epochs`): `5`. Present since the reference repository's single initial commit (`71f9fc5 DATP Artifact`); `git log -p` on this file shows no prior value — there is no history of it ever being `1` and changed to `5`, or vice versa.
- **Execution-path check.** `Makefile:196-199` (`run-regime-a` target: `$(DATP) sweep --regime=a --base-dir=$(OUTPUTS_DIR) --data-root=.`) and `COMMANDS.md` show no `local_epochs=` CLI override anywhere for the Regime A sweep. `src/datp/conf/regime/a.yaml` (the Regime-A Hydra override group) sets only `regime: a`, no federation override.
- **Proven executed value.** `outputs/results/a/{b0,b1,b2,b3,b4}/seed_{0,1,2,3,4}/resolved_config.yaml` — 25 files, one per (baseline × seed) cell of the actual Regime A confirmatory sweep, each a Hydra-written resolved-config snapshot captured at the moment that specific job actually ran. All 25 independently checked; all 25 show `local_epochs: 5`. These files are gitignored (`.gitignore:210` covers `outputs/`, confirmed via `git check-ignore`), so they are local run artifacts, not version-controlled text that could have been hand-edited after the fact. File timestamps (2026-05-13) postdate the separate `outputs/diagnostic/regime_a_seed0/` diagnostic run (2026-05-03) by ten days, consistent with this being the actual confirmatory-sweep artifact set, not a stale diagnostic leftover. MLflow run records under `mlruns/413271226137028373/` corroborate the same run set (tagged `baseline: b0` etc., `mlflow.source.name: .../cli/__main__.py`) but do not log `local_epochs` as an individual MLflow param, so they neither confirm nor contradict the resolved-config value — they are cited only as run-existence corroboration, not as independent epoch-count evidence.
- **Published value.** `paper/sections/experiments.tex:46` (prose: "clients perform $E{=}1$ local epoch per round"), `experiments.tex:47-49` (explicit deliberate rationale: "The choice $E{=}1$ keeps training dynamics conservative and fixed while isolating post-training threshold scope; evaluating larger $E$ is future work because it changes client drift and reconstruction-error geometry"), `experiments.tex:121` (reproducibility table: "Local epochs per round & 1"), `experiments.tex:126` (same table, "Seeds & $\{0, 1, 2, 3, 4\}$" — matching exactly the seed set of the resolved-config artifacts that show `5`).
- **Conflict.** The published methodology (E=1, with a deliberate stated rationale) does not match the resolved configuration that Hydra itself recorded for every one of the 25 actual Regime A confirmatory job executions (E=5). This was independently re-checked across all 5 seeds and all 5 baselines (B0–B4) — it is not a single-file anomaly. No CLI override, Makefile flag, or Regime-A config-group override that would reconcile the two was found anywhere in the reference repository.
- **What this is not.** This is not the `AnchorHistoricalCheckpointProtocol` question (N.2, accepted provisionally) — that is resolved. This is a distinct, separate historical semantic that remains genuinely contested between two evidence sources that both purport to describe the same historical anchor run.
- **Blocker state.** `ANCHOR_LOCAL_EPOCH_PROTOCOL_UNRESOLVED` — print-grade blocker. Neither value may be silently adopted as "the" historical anchor value. Recording `E=5` as recovered fact would contradict the published, peer-facing methodology; recording `E=1` as recovered fact would contradict the artifacts that actually produced the published results — either choice made without further authority would be a guess dressed as recovery.
- **What does not change.** The **new journal-extension causal ladder remains locked to E=1** regardless of this blocker — that lock is Arch §1.2's independent, non-negotiable scientific invariant for the new work, not a claim about historical reproduction, and is unaffected by whether the historical anchor actually used 1 or 5.
- **Unblock condition.** Authoritative clarification of which value governs any claim that Phase 2's anchor implementation is a faithful reproduction of the published historical protocol — e.g., an erratum/corrigendum record for the paper, an author confirmation of which artifact set is authoritative, or discovery of an overriding run configuration not found in this pass. Until then, any Phase 2 output that describes itself as reproducing "the" historical anchor protocol's local-epoch behavior must state both candidate values and their evidence rather than asserting one.
- **Affected identities.** Anchor `TrainingSpec.local_epochs` (or equivalent typed field) for `ProtocolTrack.DATP_ANCHOR`.
- **Affected tickets.** `P2-T008` (training spec — must record this as an open blocker on the anchor `local_epochs` field, not assert a recovered value), `P2-T009` (FedAvg training — implements the Arch §1.2-locked E=1 for the actual new-ladder execution regardless, per "what does not change" above; must not claim this constitutes historical reproduction), `P2-T020` (readiness evaluator must carry this as an unresolved print-grade blocker, not mark it closed).

### N.3 Authority-conflict findings (not resolved by editing Architecture/Roadmap under this ticket's scope)

1. **Local-epoch count.** Architecture §27.1 states: "The behavioral reference establishes the inherited fixed core-training profile: ...one local epoch, full participation." This claim is not supported by either the repository default or the proven-executed Regime A artifacts (both `E=5`) — see N.2b for the full evidence trail, which additionally found the *published paper* asserts `E=1`, so the true historical value is contested, not simply "E=5." Architecture §1.2 *independently* and *non-negotiably* locks E=1 as the new causal ladder's FedAvg invariant, regardless of the reference project's historical value — that lock is not in question and remains authoritative for implementation (Road/Architecture win scientific/technical conflicts by the phase-2 authority order). **Finding, not corrected here:** Architecture §27.1's phrasing should be corrected by a ticket authorized to edit `docs/DATP Core Architecture.md` (P2-T001/P2-T002 are not) to state that E=1 is a deliberate new-ladder design lock, and should not assert either E=1 or E=5 as "the" recovered historical value given N.2b's open blocker. This documentation-accuracy finding does not block Phase 2 implementation of the new ladder — P2-T008/P2-T009 must implement E=1 per Arch §1.2 regardless — but it does mean Architecture §27.1 cannot currently be cited as evidence that historical reproduction and the new ladder agree on this dimension.
2. **Anchor checkpoint schedule.** Architecture §17.1's `CheckpointDescriptor`/`CheckpointSelector` mechanism (rounds `{25,...,200}`, `regime_a_weighted_benign_validation_loss_v1`, `earliest_scheduled_round_v1`) is, per Road:295, explicitly the **journal** protocol. Architecture §17.1 does not itself state whether this exact mechanism also governs the Phase 2 ANCHOR track, and M.4 (of this log) previously assumed the anchor schedule question was simply unresolved. N.2 resolves it (accepted provisionally): the anchor route uses a distinct, single-candidate protocol. **Finding, not corrected here:** a future architecture-owning ticket should make explicit in Arch §17.1 that `ProtocolTrack.DATP_ANCHOR` uses a single-candidate checkpoint (per N.2) while `ProtocolTrack` journal tracks use the `{25,...,200}` schedule. `P2-T010` must implement the anchor-track behavior correctly regardless of whether/when Architecture prose is updated to say so explicitly.

### N.4 Print-grade blockers carried into `P2-T020` readiness

- **`ANCHOR_LOCAL_EPOCH_PROTOCOL_UNRESOLVED` (N.2b).** The historical anchor's local-epoch count is genuinely contested between the published paper (E=1, with stated rationale) and every proven-executed Regime A run artifact (E=5, unanimous across 25 cells). Open. Affects `P2-T008`, `P2-T009`, `P2-T020`. Does not block implementation of the new journal-extension ladder, which is independently locked to E=1 by Arch §1.2 regardless of this blocker's resolution.
- All other items in N.1 (rows 1–6, 8–13) were resolved with dual-sourced or otherwise unambiguous evidence and do not carry a print-grade blocker. Row 8 (`AnchorHistoricalCheckpointProtocol`, N.2) is accepted provisionally, not finally closed against further evidence.
