...
Tooling, layered skeleton, enforcement contracts, test lanes, and governance catalogue exist and pass a baseline quality gate (`P0-T026`, extended in this extraction to also require the Sonar gate `P0-T027` and the CodeScene gate `P0-T028`); no source behavior implemented yet.
...
- **Extracted in this directory.** 28 tickets (`P0-T001`–`P0-T026` preserved verbatim by canonical ID, plus two added tickets, `P0-T027` and `P0-T028`).
...
- **Added ticket and justification.** `P0-T028` — Configure CodeScene analysis and quality gate. Added under the identical one-tool-per-ticket precedent established by `P0-T027`: CodeScene is a second, independent code-health/hotspot/complexity analysis tool with its own project configuration (`.codescene/code-health-rules.json`) and its own Nox session, and no existing Phase 0 ticket (including `P0-T027` itself) can absorb it without overloading a single-tool-configuration responsibility. `P0-T028` is a peer of `P0-T027`, not a dependent of it: both depend on the same base tooling (`P0-T007`, `P0-T008`, `P0-T009`), and neither depends on the other. `P0-T028` is genuinely new: no existing ID was renumbered, reused, or altered, and `docs/MASTER_TICKET_LOG.md` itself was not modified — the addition exists only under `docs/tickets/`.

## Phase-gate ticket

`P0-T026` — Establish implementation-task governance and repository baseline quality gate. Depends directly on every one of `P0-T001`–`P0-T025` (per `docs/MASTER_TICKET_LOG.md`) plus `P0-T027` and `P0-T028` (added during extraction so neither the Sonar gate nor the CodeScene gate can be bypassed); blocks `P1-T001` (outside this phase's extraction scope). Ruff, Pyright strict, Pylance-configuration parity, import-linter contracts, pytest-archon, Nox sessions, the Sonar quality gate, the CodeScene quality gate, and thin governance adapters must all pass green before `P0-T026` may become `DONE`.

## Ordered ticket table

Ordered by canonical ID; the `Order note` column records the actual dependency-topological position where it differs from ID order (relevant only for `P0-T027` and `P0-T028`).
...
| [P0-T007](P0-T007.md) | Configure Ruff lint and format | foundation | P1-Mandatory | P0-T002 | P0-T026, P0-T027, P0-T028 | — |
| [P0-T008](P0-T008.md) | Configure Pyright strict typing | foundation | P0-Blocking | P0-T002 | P0-T026, P0-T027, P0-T028 | Also owns Pylance-configuration parity (Section 15 of the conversion task). |
| [P0-T009](P0-T009.md) | Configure pytest, coverage, timeout, and order-randomization | foundation | P0-Blocking | P0-T002 | P0-T010, P0-T026, P0-T027, P0-T028 | — |
...
| [P0-T026](P0-T026.md) | Establish implementation-task governance and repository baseline quality gate | foundation | P0-Blocking | P0-T001–P0-T025, **P0-T027**, **P0-T028** | P1-T001 | Phase gate. Dependency on `P0-T027` and `P0-T028` added during extraction. Owns ticket-status lifecycle governance. |
...
| [P0-T028](P0-T028.md) | Configure CodeScene analysis and quality gate | foundation | P0-Blocking | P0-T007, P0-T008, P0-T009 | P0-T026 | **Added ticket** (not in `docs/MASTER_TICKET_LOG.md`); see "Added ticket and justification" above. Owns the CodeScene quality gate. Peer of `P0-T027`, not dependent on it. |
...
- **Added: `P0-T028`.** See "Added ticket and justification" above. No ticket was split; every original `P0-T001`–`P0-T026` responsibility maps to exactly one ticket, unchanged.
...
`P0-T001` through `P0-T026` are preserved exactly as they appear in `docs/MASTER_TICKET_LOG.md` Section G (title, type, priority, scientific-execution classification, campaign scope, dependencies, blocks, roadmap IDs) and Section H (all 38 template fields), with the single documented exception that `P0-T026`'s `Dependencies` field is extended to include the added `P0-T027` and `P0-T028`. No ticket was renumbered, no retired ID was reused (Phase 0 has no retired ID), and no ticket was moved into or out of Phase 0.

**Resolved authority-document inconsistency (historical note).** A prior revision of `docs/MASTER_TICKET_LOG.md` referred to its own ticket structure throughout as the "37-field template" (35 occurrences across Sections A, E, F, and L), while a direct mechanical count of Section H's own defined template is 38 named fields (Ticket ID through Final review checklist). This was a self-description miscount in the master log, not a structural or content defect — no ticket's field set, order, or content was ever wrong. It has since been corrected directly in `docs/MASTER_TICKET_LOG.md` (all 35 occurrences updated to "38-field"/"38 field"), with the correction itself recorded in that document's own Section L changelog, consistent with the same historical-correction discipline already used there for the Section G priority fix noted below. This README's own "38 template fields" statement above already matched the actual field count before and after that correction.

## Responsibility ownership decisions (cross-cutting requirements)

Every standalone ticket file requires the same universal governance content (lifecycle checklist, repository-wide post-implementation audit, architecture boundary audit, raw-dictionary audit, ticket/document-reference-in-code prohibition, documentation/comment audit, full validation list, Pyright/Pylance requirements, Sonar and CodeScene requirements, determinism/reproducibility audit, and the three scientific-drift audits). Beyond that universal content, the following Phase 0 tickets are the canonical *mechanism* owners for the corresponding cross-cutting responsibility, i.e. the ticket whose deliverable is the tool, hook, or gate that makes the requirement enforceable for every ticket in every later phase:
...
| CodeScene quality gate | `P0-T028` | New ticket, peer of `P0-T027`; see justification above. |
