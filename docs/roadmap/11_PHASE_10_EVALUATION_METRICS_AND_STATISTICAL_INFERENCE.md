...
## Reproducible CLI boundary

Phase 10 exposes `run-confirmatory-seed --training-seed <seed>`, `run-confirmatory-grid`, and `analyze-confirmatory-grid`. The first two rebuild the declared N-BaIoT FedAvg path through preprocessing, training, non-test checkpoint selection, scoring, feasible threshold methods, and Phase 10 evaluation; the analysis command reloads only typed completed evaluation documents and publishes the declared paired inference. The grid is limited to the declared ten confirmatory seeds; none of these commands executes Phase 11 external or temporal work. `materialize-canonical-datasets` remains the required first command after an output reset.

## Implementation record — complete

- Entry audit: Phase 09 is `COMPLETE`; its immutable score manifests provide checkpoint, preprocessing, calibration-score, and evaluation-score checksums. The protocol graph resolves the near-zero warning (`0.01`), temporal materiality (`0.10`), ten paired seeds, 95% confidence level, and 10,000 paired BCa resamples.
- Implemented source surface: all listed evaluation and analysis modules plus the two Phase 10 orchestration stages. The operating rule is exclusively `reconstruction_error > threshold`; exact-boundary scores are benign. Per-client FPR/TPR/balanced accuracy/binary Macro-F1/AUROC preserve typed unavailable or undefined outcomes. Population FPR dispersion is unweighted and uses `ddof=0`; CV has no denominator stabilizer and attaches the protocol-driven near-zero warning.
- Diagnostics: held-out benign conformal coverage, threshold-estimation error/attainment/nested-replicate variance, estimated serialized payload accounting, and evidence-gated alert burden were added. Alert burden remains suppressed without traffic-rate evidence. Fixed-score checksum and per-client AUROC invariance controls reject mismatches.
- Analysis: seed-paired BCa, Wilcoxon, matched-pairs rank-biserial, Holm adjustment, deterministic descriptive summaries, typed JSD blocker, persisted-cluster mechanism summaries, temporal recovery quantities, and narrow scientific decisions were added. No Phase 11 execution was added.
- Validation so far: source-tree lock and existing cohort tests pass (`11 passed`); focused compilation, Ruff, and Pyright for the changed Phase 10 modules pass; a direct boundary smoke check verified `score == threshold` is benign and produced `TN=2, FP=0, TP=2, FN=0`.
- Verification update: all named Phase 10 unit, property, and scientific-contract tests now exist; `pytest -n 6 -q` passed with `616 passed`. Scoped Ruff and Pyright pass, and import-linter keeps all four contracts. The required Phase 10 evaluation was deliberately not published from the legacy real score directory: its immutable score files remain readable, but it lacks the persisted score manifest, threshold artifact, eligibility manifest, and split-manifest checksum needed to reconstruct a provenance-complete request without inventing evidence. A read-only diagnostic on the legacy scores found shared-threshold mean FPR `0.0416577729` and local-threshold mean FPR `0.0500033164`; it is explicitly non-confirmatory. Phase 10 remains `IN_PROGRESS` pending a provenance-complete real rerun and the external code-health gate. Pylint reports only reviewed duplicate-code findings across deliberately parallel typed validators and result constructors (`9.94/10`); no correctness or scientific-contract defect was found.
- Closure update: after the authorized output reset, all ten declared seeds produced provenance-complete threshold and evaluation artifacts for every feasible method (`shared_threshold`, `local_threshold`, `family_threshold`, and `cluster_threshold`). `analyze-confirmatory-grid` reloaded the ten shared/local documents and published the required paired result: mean shared-minus-local `CV(FPR)` difference `0.5860661290246958`, 95% BCa interval `[0.5712251566051214, 0.6004196649849424]`, decision `supported`; a second invocation reused the analysis artifact. The full suite passes with `617 passed`; repository-wide Ruff formatting/lint, Pyright, and all four import-linter contracts pass. Pylint's project score is `9.91/10`; its remaining reports are reviewed pre-existing complexity, import-placement, and intentionally parallel typed-record similarity findings, with no Phase 10-specific defect. Credential-safe SonarQube authentication succeeded with zero secret issues; its Vortex agentic service returned organization-side 403 responses. CodeScene authenticated and reported complexity findings, prompting targeted simplification of the CLI and evaluation publication paths while the final neutral-path rerun proceeds.
- Final execution update: the neutral confirmatory path completed all 80 threshold/evaluation publications and one typed analysis publication under `outputs/confirmatory/`. Its analysis reran with status `reused`, preserving the same supported result and available BCa interval. The final suite remains `617 passed`; formatting, Ruff, Pyright, import-linter, and diff checks pass. CodeScene's remaining complexity signals are limited to explicit scientific validation and analysis functions; they were reviewed after targeted simplification and do not indicate a semantic or architecture defect. No Phase 11 behavior was implemented.
...
- `datp_core/analysis/inference/wilcoxon.py`
- `datp_core/analysis/inference/multiplicity.py`
- `datp_core/analysis/contrasts.py`
- `datp_core/analysis/decisions.py`
- `datp_core/analysis/mechanisms/association.py`
- `datp_core/analysis/mechanisms/clustering.py`
- `datp_core/analysis/mechanisms/dispersion.py`
- `datp_core/analysis/mechanisms/divergence.py`
- `datp_core/analysis/mechanisms/movement.py`
- `datp_core/analysis/mechanisms/absorption.py`
