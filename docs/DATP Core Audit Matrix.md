...
| ID | Source line | Audit requirement | Implementation disposition | Audit outcome | Evidence / remediation |
|---|---:|---|---|---|---|
| `GATE-F-001` | 6061 | Every **federated training** execution has exactly one scientific terminal detector at round `200`; `FEDAVG_LOCAL_FINE_TUNING` starts from that detector and produces separately identified post-training client-personalized states after exactly ten local epochs. | `NOT_AUDITED` | — | — |
| `GATE-F-002` | 6062 | Recovery checkpoints are used only to resume interrupted execution. | `NOT_AUDITED` | — | — |
| `GATE-F-003` | 6063 | Diagnostic checkpoints are observational and never become score sources. | `NOT_AUDITED` | — | — |
| `GATE-F-004` | 6064 | SHARED_THRESHOLD/LOCAL_THRESHOLD/FAMILY_THRESHOLD/CLUSTER_THRESHOLD do not trigger policy-specific retraining. | `NOT_AUDITED` | — | — |
...
| `GATE-F-011` | 6072 | `FEDAVG_LOCAL_FINE_TUNING` initializes every client from the exact seed-matched FedAvg round-200 model, uses a fresh optimizer state, exactly 10 benign-training epochs, no early stopping, and no calibration/evaluation/attack-label access. | `NOT_AUDITED` | — | — |
...
| ID | Source line | Audit requirement | Implementation disposition | Audit outcome | Evidence / remediation |
|---|---:|---|---|---|---|
| `GATE-J-001` | 6127 | Exact pooled benign quantile uses the type-7 pooled oracle. | `NOT_AUDITED` | — | — |
| `GATE-J-002` | 6128 | Sample-weighted shared construction uses the declared eligible calibration weights. | `NOT_AUDITED` | — | — |
| `GATE-J-003` | 6129 | Fixed shrinkage executes the full λ curve and never selects a winner post hoc. | `NOT_AUDITED` | — | — |
| `GATE-J-004` | 6130 | Size-aware shrinkage uses `n_k_used/(n_k_used+100)` and never substitutes `n_k_source` for `m` in a subsampled cell. | `NOT_AUDITED` | — | — |
| `GATE-J-005` | 6131 | `FEDERATED_BENIGN_SUMMARY_THRESHOLD` communicates only the predeclared benign summaries and includes the full pooled variance decomposition. | `NOT_AUDITED` | — | — |
| `GATE-J-006` | 6132 | `FEDERATED_BENIGN_SUMMARY_THRESHOLD` is never called Laridi-faithful. | `NOT_AUDITED` | — | — |
| `GATE-J-007` | 6133 | KLL uses float64, canonical `k=400`, sensitivity `{200,800}`, ascending client merge order, and the locked inclusive-rank semantics. | `NOT_AUDITED` | — | — |
| `GATE-J-008` | 6134 | KLL observed empirical rank/threshold errors are measured against the exact pooled type-7 oracle. | `NOT_AUDITED` | — | — |
| `GATE-J-009` | 6135 | KLL implementation randomness follows Part II §9.2 and remains nested within training seed. | `NOT_AUDITED` | — | — |
| `GATE-J-010` | 6136 | `MEAN_PLUS_STANDARD_DEVIATION_ESTIMATOR` uses float64, arithmetic mean, sample standard deviation with `ddof=1`, and the locked `{shared, local}` 2×2 scope comparison; it is never presented as a faithful reproduction of Meidan's complete detector. | `NOT_AUDITED` | — | — |
| `GATE-J-011` | 6137 | LOCAL_CONFORMAL_THRESHOLD reports held-out benign coverage and its limitations; it does not claim arbitrary client-conditional validity. | `NOT_AUDITED` | — | — |
...
| ID | Source line | Audit requirement | Implementation disposition | Audit outcome | Evidence / remediation |
|---|---:|---|---|---|---|
| `GATE-M-001` | 6170 | Training seed is the independent inferential unit. | `NOT_AUDITED` | — | — |
| `GATE-M-002` | 6171 | Nested replicates are summarized within seed before across-seed inference. | `NOT_AUDITED` | — | — |
| `GATE-M-003` | 6172 | Confirmatory delta direction matches the Part III definition. | `NOT_AUDITED` | — | — |
| `GATE-M-004` | 6173 | The confirmatory statistic is the arithmetic mean of the ten paired seed-level deltas. | `NOT_AUDITED` | — | — |
| `GATE-M-005` | 6174 | The confirmatory uncertainty is the locked two-sided 95% BCa interval over paired seed deltas. | `NOT_AUDITED` | — | — |
| `GATE-M-006` | 6175 | Degenerate/invalid BCa states produce `CONFIRMATORY_INFERENCE_UNAVAILABLE` rather than a substituted method. | `NOT_AUDITED` | — | — |
| `GATE-M-007` | 6176 | Wilcoxon is paired, uses exact computation where feasible, and records fallback/approximation behavior. | `NOT_AUDITED` | — | — |
| `GATE-M-008` | 6177 | Rank-biserial effect size is the matched-pairs version, not unpaired Cliff's delta. | `NOT_AUDITED` | — | — |
| `GATE-M-009` | 6178 | Secondary emphasized p-values use predeclared families and Holm correction. | `NOT_AUDITED` | — | — |
| `GATE-M-010` | 6179 | Exact paired sign-test uses only non-zero paired deltas, an exact `Binomial(n_nonzero, 0.5)` null, and no normal approximation; zero deltas remain visible in sign counts. | `NOT_AUDITED` | — | — |
| `GATE-M-011` | 6180 | Leave-one-seed-out precision diagnostics are reported without changing the inferential sample. | `NOT_AUDITED` | — | — |
| `GATE-M-012` | 6181 | Leave-one-device-out confirmatory influence uses the same ten training seeds and already generated scores; the nine omitted-device means are dependent diagnostics and are never treated as nine independent replicates. | `NOT_AUDITED` | — | — |
| `GATE-M-013` | 6182 | `LODO_HIGH_INFLUENCE` is evaluated exactly as Part III §15.1A specifies; the 25% influence boundary is descriptive and never modifies the BCa decision rule. | `NOT_AUDITED` | — | — |
| `GATE-M-014` | 6183 | No seed or client is removed because of effect direction. | `NOT_AUDITED` | — | — |
...
| `GATE-N-008` | 6194 | Natural-device mechanism leave-one-device-out analysis never retrains, refits preprocessing, or rescores; it recomputes only the population-dependent heterogeneity/shared-threshold quantities defined in Part II §7.4. | `NOT_AUDITED` | — | — |
| `GATE-N-009` | 6195 | The `9 × 10` leave-one-device seed cells are never treated as 90 independent observations; the association influence analysis remains a sensitivity analysis over the original population/seed structure. | `NOT_AUDITED` | — | — |
| `GATE-N-010` | 6196 | Part II §7.5 reports exact per-seed FPR/TPR direction counts without inventing a floating tolerance or post-hoc materiality cutoff. | `NOT_AUDITED` | — | — |
| `GATE-N-011` | 6197 | Part II §7.5A support-versus-burden Spearman coefficients use only the common valid client set, require at least five clients plus nonconstant inputs, use average ranks for ties, and do not report client-level inferential p-values from the nine-device population. | `NOT_AUDITED` | — | — |
| `GATE-N-012` | 6198 | The support-versus-burden diagnostic is interpreted associatively and never used to claim that calibration support causes client harm. | `NOT_AUDITED` | — | — |
| `GATE-N-013` | 6199 | Equity–utility Pareto analysis never invents a scalarized winner. | `NOT_AUDITED` | — | — |
| `GATE-N-014` | 6201 | Every FedProx, `FEDAVG_LOCAL_FINE_TUNING`, and Ditto stress condition reports the common Part I §7.2B score/threshold-alignment tuple whenever inputs are valid. | `NOT_AUDITED` | — | — |
| `GATE-N-015` | 6202 | `ScopeAbsorption` and every `AlignmentReduction` are un-clipped; non-positive FedAvg denominators produce the declared unavailable states rather than an epsilon adjustment. | `NOT_AUDITED` | — | — |
| `GATE-N-016` | 6203 | `FEDAVG_LOCAL_FINE_TUNING` uses exactly ten benign-training local epochs from the exact round-200 FedAvg weights, a fresh optimizer state, no early stopping, and no calibration/evaluation/attack-label access. | `NOT_AUDITED` | — | — |
| `GATE-N-017` | 6204 | The N-BaIoT helped/harmed profile reports all ten seed-level fractions and every physical-device help/harm frequency; the 9×10 cells are never treated as independent observations. | `NOT_AUDITED` | — | — |
| `GATE-N-018` | 6205 | Calibration-support strata are frozen from ascending `SupportScore_k=median_s(n_{s,k,source})` with canonical-client-ID tie-break into ranks `1..3`, `4..6`, `7..9`; if exactly nine eligible N-BaIoT clients are not available, the stratum analysis emits the declared unavailable state. | `NOT_AUDITED` | — | — |
| `GATE-N-019` | 6206 | The empirical policy-selection surface emits only the declared typed states and raw nondominated sets; no learned classifier, cutoff, scalar utility weight, or post-hoc production rule is fitted. | `NOT_AUDITED` | — | — |
| `GATE-N-020` | 6207 | `H_TAUTOLOGY` reporting uses disjoint calibration/evaluation row identities and shows calibration exceedance, held-out target error, and calibration-generalization gap rather than calling local q95 held-out FPR “guaranteed.” | `NOT_AUDITED` | — | — |
...
| `THRESHOLD-310` | IV | 6064 | 8. Gate F — Training and terminal-detector integrity | SHARED_THRESHOLD/LOCAL_THRESHOLD/FAMILY_THRESHOLD/CLUSTER_THRESHOLD do not trigger policy-specific retraining. | `NOT_AUDITED` | — |
...
| `THRESHOLD-326` | IV | 6130 | 12. Gate J — Comparator and threshold-variant integrity | Size-aware shrinkage uses `n_k_used/(n_k_used+100)` and never substitutes `n_k_source` for `m` in a subsampled cell. | `NOT_AUDITED` | — |
| `THRESHOLD-327` | IV | 6131 | 12. Gate J — Comparator and threshold-variant integrity | `FEDERATED_BENIGN_SUMMARY_THRESHOLD` communicates only the predeclared benign summaries and includes the full pooled variance decomposition. | `NOT_AUDITED` | — |
| `THRESHOLD-328` | IV | 6132 | 12. Gate J — Comparator and threshold-variant integrity | `FEDERATED_BENIGN_SUMMARY_THRESHOLD` is never called Laridi-faithful. | `NOT_AUDITED` | — |
| `THRESHOLD-329` | IV | 6133 | 12. Gate J — Comparator and threshold-variant integrity | KLL uses float64, canonical `k=400`, sensitivity `{200,800}`, ascending client merge order, and the locked inclusive-rank semantics. | `NOT_AUDITED` | — |
| `THRESHOLD-330` | IV | 6134 | 12. Gate J — Comparator and threshold-variant integrity | KLL observed empirical rank/threshold errors are measured against the exact pooled type-7 oracle. | `NOT_AUDITED` | — |
| `THRESHOLD-331` | IV | 6135 | 12. Gate J — Comparator and threshold-variant integrity | KLL implementation randomness follows Part II §9.2 and remains nested within training seed. | `NOT_AUDITED` | — |
| `THRESHOLD-332` | IV | 6136 | 12. Gate J — Comparator and threshold-variant integrity | `MEAN_PLUS_STANDARD_DEVIATION_ESTIMATOR` uses float64, arithmetic mean, sample standard deviation with `ddof=1`, and the locked `{shared, local}` 2×2 scope comparison; it is never presented as a faithful reproduction of Meidan's complete detector. | `NOT_AUDITED` | — |
| `CALIBRATION-236` | IV | 6137 | 12. Gate J — Comparator and threshold-variant integrity | LOCAL_CONFORMAL_THRESHOLD reports held-out benign coverage and its limitations; it does not claim arbitrary client-conditional validity. | `NOT_AUDITED` | — |
...
| `SCORE-031` | IV | 6162 | 14. Gate L — Evaluation and metric integrity | AUROC/AP are detector-quality controls and do not become threshold-scope verdicts. | `NOT_AUDITED` | — |
| `CALIBRATION-237` | IV | 6163 | 14. Gate L — Evaluation and metric integrity | Held-out target-attainment error is computed from held-out benign rows and is never replaced by calibration-set exceedance. | `NOT_AUDITED` | — |
| `CALIBRATION-238` | IV | 6164 | 14. Gate L — Evaluation and metric integrity | Calibration-to-held-out benign generalization gap uses the exact calibration scores that constructed each scalar threshold, the strict `score > threshold` exceedance rule, and the unchanged held-out benign evaluation rows. | `NOT_AUDITED` | — |
...
| `TRAIN-091` | IV | 6170 | 15. Gate M — Statistical integrity | Training seed is the independent inferential unit. | `NOT_AUDITED` | — |
| `STAT-023` | IV | 6171 | 15. Gate M — Statistical integrity | Nested replicates are summarized within seed before across-seed inference. | `NOT_AUDITED` | — |
| `STAT-024` | IV | 6172 | 15. Gate M — Statistical integrity | Confirmatory delta direction matches the Part III definition. | `NOT_AUDITED` | — |
| `STAT-025` | IV | 6173 | 15. Gate M — Statistical integrity | The confirmatory statistic is the arithmetic mean of the ten paired seed-level deltas. | `NOT_AUDITED` | — |
| `STAT-026` | IV | 6174 | 15. Gate M — Statistical integrity | The confirmatory uncertainty is the locked two-sided 95% BCa interval over paired seed deltas. | `NOT_AUDITED` | — |
| `STAT-027` | IV | 6175 | 15. Gate M — Statistical integrity | Degenerate/invalid BCa states produce `CONFIRMATORY_INFERENCE_UNAVAILABLE` rather than a substituted method. | `NOT_AUDITED` | — |
| `STAT-028` | IV | 6176 | 15. Gate M — Statistical integrity | Wilcoxon is paired, uses exact computation where feasible, and records fallback/approximation behavior. | `NOT_AUDITED` | — |
| `STAT-029` | IV | 6177 | 15. Gate M — Statistical integrity | Rank-biserial effect size is the matched-pairs version, not unpaired Cliff's delta. | `NOT_AUDITED` | — |
| `STAT-030` | IV | 6178 | 15. Gate M — Statistical integrity | Secondary emphasized p-values use predeclared families and Holm correction. | `NOT_AUDITED` | — |
| `STAT-031` | IV | 6179 | 15. Gate M — Statistical integrity | Exact paired sign-test uses only non-zero paired deltas, an exact `Binomial(n_nonzero, 0.5)` null, and no normal approximation; zero deltas remain visible in sign counts. | `NOT_AUDITED` | — |
| `STAT-032` | IV | 6180 | 15. Gate M — Statistical integrity | Leave-one-seed-out precision diagnostics are reported without changing the inferential sample. | `NOT_AUDITED` | — |
| `TRAIN-092` | IV | 6181 | 15. Gate M — Statistical integrity | Leave-one-device-out confirmatory influence uses the same ten training seeds and already generated scores; the nine omitted-device means are dependent diagnostics and are never treated as nine independent replicates. | `NOT_AUDITED` | — |
| `STAT-033` | IV | 6182 | 15. Gate M — Statistical integrity | `LODO_HIGH_INFLUENCE` is evaluated exactly as Part III §15.1A specifies; the 25% influence boundary is descriptive and never modifies the BCa decision rule. | `NOT_AUDITED` | — |
| `STAT-034` | IV | 6183 | 15. Gate M — Statistical integrity | No seed or client is removed because of effect direction. | `NOT_AUDITED` | — |
...
| `PREPROCESS-055` | IV | 6194 | 16. Gate N — Mechanism-analysis integrity | Natural-device mechanism leave-one-device-out analysis never retrains, refits preprocessing, or rescores; it recomputes only the population-dependent heterogeneity/shared-threshold quantities defined in Part II §7.4. | `NOT_AUDITED` | — |
| `DATASET-099` | IV | 6195 | 16. Gate N — Mechanism-analysis integrity | The `9 × 10` leave-one-device seed cells are never treated as 90 independent observations; the association influence analysis remains a sensitivity analysis over the original population/seed structure. | `NOT_AUDITED` | — |
| `METRIC-037` | IV | 6196 | 16. Gate N — Mechanism-analysis integrity | Part II §7.5 reports exact per-seed FPR/TPR direction counts without inventing a floating tolerance or post-hoc materiality cutoff. | `NOT_AUDITED` | — |
| `CALIBRATION-240` | IV | 6197 | 16. Gate N — Mechanism-analysis integrity | Part II §7.5A support-versus-burden Spearman coefficients use only the common valid client set, require at least five clients plus nonconstant inputs, use average ranks for ties, and do not report client-level inferential p-values from the nine-device population. | `NOT_AUDITED` | — |
| `CALIBRATION-241` | IV | 6198 | 16. Gate N — Mechanism-analysis integrity | The support-versus-burden diagnostic is interpreted associatively and never used to claim that calibration support causes client harm. | `NOT_AUDITED` | — |
| `GLOBAL-128` | IV | 6199 | 16. Gate N — Mechanism-analysis integrity | Equity–utility Pareto analysis never invents a scalarized winner. | `NOT_AUDITED` | — |
| `THRESHOLD-335` | IV | 6201 | 16. Gate N — Mechanism-analysis integrity | Every FedProx, `FEDAVG_LOCAL_FINE_TUNING`, and Ditto stress condition reports the common Part I §7.2B score/threshold-alignment tuple whenever inputs are valid. | `NOT_AUDITED` | — |
| `BOUNDARY-009` | IV | 6202 | 16. Gate N — Mechanism-analysis integrity | `ScopeAbsorption` and every `AlignmentReduction` are un-clipped; non-positive FedAvg denominators produce the declared unavailable states rather than an epsilon adjustment. | `NOT_AUDITED` | — |
| `CALIBRATION-242` | IV | 6203 | 16. Gate N — Mechanism-analysis integrity | `FEDAVG_LOCAL_FINE_TUNING` uses exactly ten benign-training local epochs from the exact round-200 FedAvg weights, a fresh optimizer state, no early stopping, and no calibration/evaluation/attack-label access. | `NOT_AUDITED` | — |
| `DATASET-100` | IV | 6204 | 16. Gate N — Mechanism-analysis integrity | The N-BaIoT helped/harmed profile reports all ten seed-level fractions and every physical-device help/harm frequency; the 9×10 cells are never treated as independent observations. | `NOT_AUDITED` | — |
...
| `GLOBAL-129` | IV | 6206 | 16. Gate N — Mechanism-analysis integrity | The empirical policy-selection surface emits only the declared typed states and raw nondominated sets; no learned classifier, cutoff, scalar utility weight, or post-hoc production rule is fitted. | `NOT_AUDITED` | — |
| `CALIBRATION-244` | IV | 6207 | 16. Gate N — Mechanism-analysis integrity | `H_TAUTOLOGY` reporting uses disjoint calibration/evaluation row identities and shows calibration exceedance, held-out target error, and calibration-generalization gap rather than calling local q95 held-out FPR “guaranteed.” | `NOT_AUDITED` | — |
...
## 18. Four independent matrix audits

### AUDIT 1 — Structural source-coverage audit

Checks performed:

- every H2–H4 roadmap heading through Part IV is in the source-coverage ledger;
- every numbered/named bold contract anchor is in the source-coverage ledger;
- every Part II master experiment-index row is in the experiment catalogue;
- every Part IV Gate A–R checkbox is in the gate matrix;
- every extracted list/procedure item is in the atomic requirement register;
- every prose-like semantic statement through Part IV omitted by other extractors is represented in Section 15A (`911` sentinels);
- every roadmap table row through Part IV not already reproduced verbatim is represented in Section 15B (`128` sentinels);
- section-title/anchor coverage is explicitly non-substitutive for executable closure.

**Result:** `PASS`

### AUDIT 2 — Numerical / formula / literal-lock audit

Checks performed:

- all `198` display-math blocks are present in the formula ledger with exact source text and line ranges;
- all `93` fenced literal/code blocks are present in the literal ledger;
- all `30` rows from the roadmap numerical navigation ledger are reproduced;
- central high-risk locks are independently asserted: q95/type-7, ten confirmatory seeds, round 200, one local epoch, eligibility 100, calibration replicate count 10, FedProx μ grid, Ditto λD grid, ten fine-tuning epochs, KLL k grid, temporal materiality/recovery values, and serialization tolerance semantics.

**Result:** `PASS`

### AUDIT 3 — Experiment → evidence completeness audit

Checks performed:

- all `36` Part II master-index experiment/analysis rows have an implementation card;
- each card preserves every list/procedure item detected inside its authoritative section;
- Part III metric/statistical sections are separately catalogued;
- mandatory figure/table contracts are separately represented;
- Gates K/L/M/N/O/P provide independent completeness checks for experiments, metrics, statistics, mechanisms, external/boundary, and temporal evidence.

**Result:** `PASS`

### AUDIT 4 — Scientific-drift / claim-boundary audit

Checks performed independently of the three coverage audits:

- sole confirmatory endpoint remains N-BaIoT natural devices, SHARED_THRESHOLD vs LOCAL_THRESHOLD, `CV(FPR)`, ten paired seeds, locked BCa;
- training/model/preprocessing stress conditions remain outside the core fixed-score causal ladder;
- AUROC/AP remain detector controls, not threshold verdicts;
- Edge client attack-sensitive metrics remain unavailable where assignment is invalid;
- CICIoT2023 remains file-defined pseudo-client boundary evidence only;
- persistent identifiable-client/full-participation regime remains explicit;
- no attack/Byzantine/privacy/hardware/fleet-scale/continuous-drift claim is introduced;
- current descriptive identifiers are used; no active opaque B-number or lettered population alias is introduced by the matrix;
- optional analyses remain optional and cannot rescue mandatory/confirmatory failure;
- prose-only positive definitions, interpretation rules, scope/claim boundaries, and protocol constraints are independently sentinelized rather than relying on section-heading coverage;
- source tables encoding claim boundaries/reviewer objections/evidence roles are independently sentinelized rather than treated as decorative prose.

**Result:** `PASS`

### 18.5 Audit caveat

These four audits prove **matrix extraction fidelity against the locked roadmap snapshot**, not repository correctness. The source filename has also been reconciled to `Journal_Extension_Master_Roadmap(20260812-161910).md`; the content hash remains exactly the locked value. Repository statuses remain `NOT_AUDITED` until the agent maps and verifies the actual DATP-Core code and artifacts.

## 19. Matrix generation statistics

| Object | Count |
|---|---:|
| Roadmap lines | 6484 |
| H2–H4 headings through Part IV | 81 |
| Numbered/named bold contract anchors through Part IV | 220 |
| Source-coverage anchors | 301 |
| Display-math blocks | 198 |
| Fenced literal/code blocks | 93 |
| Numerical navigation-ledger rows | 30 |
| Part II experiment-index rows | 36 |
| Part III numbered metric/stat/temporal subcontracts | 65 |
| Part IV Gate A–R checkbox items | 205 |
| Atomic list/procedure requirements | 1195 |
| Prose-only semantic contract sentinels added | 911 |
| Source-table rows requiring explicit semantic sentinels | 128 |
| Typed unavailability/status tokens detected | 23 |
