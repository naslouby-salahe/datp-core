...
| `external_device_dataset_validation` | **suppressed** (`attack_row_client_assignment_unavailable`): the intended full Edge-IIoTset chain with `FederatedSummaryStatisticThreshold` and q-sweep is non-executable because attack rows cannot be assigned to the benign sensor groups | external partition (group granularity authorized, device rejected), `FederatedSummaryStatisticThreshold` | none produced (suppressed) | external CONFIRMATORY_INTERVAL (intended) |
