...
| Edge-IIoTset `Temperature_and_Humidity` subnet-consistency anomaly | source inspection | `edge_iiotset_client_granularity_feasibility` | `RESOLVED`; the full-corpus direction-normalized audit shows `Temperature_and_Humidity` is a clean subnet-0 folder (192.168.0.x, shared with `Modbus`); the earlier sampled "not consistent / public-IP" impression was a MAC-address-source artifact (17% of rows), not a real anomaly (`CONFIGURATION_AND_EXPERIMENT_CATALOGUE.md §11.3`) |
